...
def analyze_pawns(board):
    results = {
        'white': {'passed': [], 'isolated': [], 'king_defenders': [], 'central': [], 'doubled': [], 'advanced': []},
        'black': {'passed': [], 'isolated': [], 'king_defenders': [], 'central': [], 'doubled': [], 'advanced': []}
    }
        ...
        for square in board.pieces(chess.PAWN, color):
            ...
            for forward_rank in range(rank + 1, 8) if color == chess.WHITE else range(rank - 1, -1, -1):
                if any(board.piece_at(chess.square(f, forward_rank)) for f in [file-1, file, file+1] if 0 <= f < 8):
                    ...
                    break
            if is_passed:
                results[color_str]['passed'].append(chess.square_name(square))

            # Check for isolated pawns возможно неправильно считает
            is_isolated = not any(board.piece_at(chess.square(f, rank)) for f in [file - 1, file + 1] if 0 <= f < 8)
            if is_isolated:
                results[color_str]['isolated'].append(chess.square_name(square))

            # Check for king defenders
            king_square = board.king(color)
            if king_square and abs(chess.square_rank(king_square) - rank) <= 1:
                results[color_str]['king_defenders'].append(chess.square_name(square))

            # Check for central pawns
            if file in [3, 4]:  # Files D and E
                results[color_str]['central'].append(chess.square_name(square))
            ...
            for r in range(8):  # Просмотр всех рядов на файле
                if r != rank and board.piece_at(chess.square(file, r)) == chess.Piece(chess.PAWN, color):
                    ...
                    break
            if is_doubled:
                results[color_str]['doubled'].append(chess.square_name(square))

            # Check for advanced pawns
            if (color == chess.WHITE and rank >= 5) or (color == chess.BLACK and rank <= 2):
                results[color_str]['advanced'].append(chess.square_name(square))
